...
iscDF<- read.csv("monitoring_rebio_ISC_2020_05_06.csv", header=T, sep=";", dec=".", na.strings = "NA", stringsAsFactors = T) #check if it's possible to download data from google drive  
subsDF<- read.csv("monitoring_rebio_SUBS_2020_05_06.csv", header=T, sep=";", dec=".")
iscDF
subsDF
...
taxaquadrat = iscDF %>% filter(taxon!="Lixo_de_pesca", taxon!="Lixo_comun", abund==0) %>% # cut the 0 abund on data
  group_by(site_code,habitat, trans_n)%>% 
...
pp = pp + geom_boxplot() + ylab("Taxa per transect(n)") + 
  ...
  theme_bw(base_size = 9) + theme(legend.position = "none") #order plot by latitude
...
taxaTable = iscDF %>% 
  dplyr::group_by(AphiaID) %>% dplyr::filter(!is.na(AphiaID) & abund!="0") %>%  # zeros taxa out of data frame
